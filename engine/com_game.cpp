...
void Game::loadResources(std::string dir, ResourceTypeEnum resType)
{
	...
	try
	{
		for (directory_iterator itr(dir); itr != end_itr; itr++)
		{
			...
			{
				loadResources(itr->path().file_string(), resType);
			}
			else
			{
				switch (resType)
				{
                    case SPRITE:
                    {
                        spriteManager.load(itr->path().file_string().c_str());
                        break;
                    }
                    ...
                    case TEXTURE:
                    {
                        textureManager.load(itr->path().file_string().c_str());
                        break;
                    }

                    case TILESET:
                    {
                        tilesetManager.load(itr->path().file_string().c_str());
                        break;
                    }

                    case FONT:
                    {
                        fontManager.load(itr->path().file_string().c_str());
                        break;
                    }
                    ...
                    case OBJECT:
                    {
                        ...
                        string temp = itr->path().file_string();
                        ...
                        if (objectElement)
                        {
                            ...
                            if (tempName.length())
                            {
                                objectList[tempName] = itr->path().file_string();
                            }
                        }
                        break;
                    }
	...
	catch (basic_filesystem_error<basic_path<std::string, path_traits> > e)
	{
